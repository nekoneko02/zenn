...
flowchart LR
    ...
    user -->|"操作"| browser
    browser -->|"1. サービスのパスワード"| service
    service -->|"2. tokenを返す"| browser
    browser -->|"3. tokenを渡す"| client
    client -->|"4. tokenは保存"| client
    client <-->|"5. 以降、tokenを使ってAPI連携する"| service
```
...
flowchart LR
    ...
    user -->|"操作"| browser
    browser -->|"1. フローの開始"| client
    client -->|"2. tokenを要求"| browser
    browser -->|"3. サービスのパスワード"| service
    service -->|"4. tokenを返す"| browser
    browser -->|"5. tokenを渡す"| client
    client -->|"6. tokenは保存"| client
    client <-->|"7. 以降、tokenを使ってAPI連携する"| service
```
...
sequenceDiagram
    ...
    user->>browser: クライアントの画面を再度表示し、tokenを入力する
...
sequenceDiagram
    ...
    user->>browser: 操作
    browser->>client: 1. フロー開始する
    client-->>browser: 2. token要求を送る（iframeあり）
    browser->>service: HTTPリクエストを送信する
    service-->>browser: ログイン画面を表示する
    browser-->>user: 「ログイン画面を埋め込んだクライアントの画面」を表示する
    user->>browser: パスワードを入力する
    browser->>service: 3. パスワードを送信する
    service-->>browser: 4. tokenを発行する
    browser->>client: 5. tokenを送信する（postMessage）
    client->>client: 6. tokenを保存する
    loop API利用
        client->>service: token付きAPI呼び出し
        service-->>client: レスポンス
...
sequenceDiagram
    ...
    user->>browser: 操作
    browser->>client: 1. フロー開始する
    client-->>browser: 2. token要求を送る（パスワード入力画面）
    browser-->>user: パスワード入力画面（クライアントの画面）を表示する
    user->>browser: パスワードを入力する
    browser->>service: JavaScriptでパスワードをサービスへ送信する
    service-->>browser: 4. tokenを発行する
    browser->>client: 5. tokenを送信する（JavaScript）
    client->>client: 6. tokenを保存する
    loop API利用
        client->>service: token付きAPI呼び出し
        service-->>client: レスポンス
